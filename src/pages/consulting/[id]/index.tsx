...
export const getServerSideProps : GetServerSideProps<ServerSideProps> = async (context) => {
  ...
  try {
    const response = await fetch(`${process.env.API_URL}/consultants/${id}`, { method: "GET" });
    if (response.status < 200 || response.status >= 400) return { notFound: true };
    const responseData = await response.json();
    return { props: { consultant: responseData, id } };
  } catch (e) {
  ...
  return (
    <div>
      ...
      <div className="md:flex-row flex flex-col gap-12 p-12">
        <div className="relative md:w-72 h-72 mr-0">
        ...
        <div className="space-y-6">
          ...
          <ol className="md:w-96 space-y-4">
            ...
            <DetailItem label="Specialization" value={consultant.specialization} />
          ...
          <p className="text-justify">{consultant.bio}</p>
